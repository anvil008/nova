...
JJ creation starts from the repository's `trunk()` revset. Git creation starts from the locally recorded `origin/HEAD`, then tries `origin/main`, `origin/master`, local `main`, and local `master`, on a new `worktree-<task>` branch. If none exists it fails and asks for `origin/HEAD` to be configured; it never falls back to an unrelated checked-out task. The hook does not fetch; refresh remote refs before starting isolation when an up-to-date remote base is required. Claude's `worktree.baseRef` setting does not control this replacement hook.
...
All parents integrate ready verified results into local main promptly; children keep intermediate commits local. Publication uses one integration bookmark and PR for accumulated results, with explicit merged/superseded branch cleanup and safe reconciliation of local-only descendants after a remote squash merge. See [the complete delivery policy](development.md#parent-owned-task-integration).
