...
Follow the user's requested scope and carry their authorization through the task. Keep the approach proportional to the change. Handle a bounded task in the current conversation; delegate only when an independent question or review warrants it. Skills contribute task-specific methods without imposing a team or delivery pipeline.
...
Choose checks that exercise the changed behavior and run the project's required final checks. Reuse results while their source and relevant environment remain unchanged. Record pre-existing failures separately; continue when they are unrelated and the changed behavior can still be verified. Stop when a failure prevents trustworthy verification of that behavior. Do not repair unrelated defects without scope authorization.

Inspect the final diff for unintended changes. Request independent review when warranted by risk or required by the project; distinguish it from self-review. Summarize the change, verification, remaining failures, and exact delivery state. Keep additional plans and evidence files only when they materially help review or resumption.
