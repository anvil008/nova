...
func agentPrompt(role Role, authority AuthorityProfile) string {
	...
	if role.Workflow != nil {
		...
		if role.Workflow.Orchestration != nil {
			...
			fmt.Fprintf(&builder, "- Goal mode: %s. Bind the objective, constraints, acceptance and verification criteria, granted authority, and stop conditions to the harness's native goal, session, thread, or resume state. If the harness has no durable primitive, preserve the same checkpoint in the current conversation and state that limitation; never claim persistence you cannot observe.\n", orchestration.GoalMode)
			fmt.Fprintf(&builder, "- Checkpoint policy: %s. At phase transitions, delegate handoffs, context compaction, interruptions, and before yielding, record the objective, constraints, decisions, completed evidence, runnable and blocked queues, active delegate identities and file ownership, and the exact next action.\n", orchestration.CheckpointPolicy)
			builder.WriteString("- On every resume, reconcile the checkpoint with the current repository and external state before releasing more work; stale delegate claims never outrank observed state.\n")
			...
			if orchestration.ModelRouting != nil {
				...
				fmt.Fprintf(&builder, "- Supervisor bootstrap: the parent starts or uses the authenticated loopback service with `%s serve`. Its default URL is `%s`, its state directory is `%s`, and its default bearer-token file is `%s`. Supported overrides are `%s`, `%s`, `%s`, and `%s`.\n", runplane.Binary, runplane.DefaultURL, runplane.DefaultState, runplane.DefaultTokenFile, runplane.StateEnv, runplane.URLEnv, runplane.TokenEnv, runplane.TokenFileEnv)
				...
				fmt.Fprintf(&builder, "- Start a foreign run with `%s start --request route.json`. Put the bounded role/task brief in the request file (`%s`) and send follow-up or resume input through stdin, never argv.\n", runplane.Binary, runplane.BriefTransport)
				fmt.Fprintf(&builder, "- Supported lifecycle commands are `%s health`, `%s capabilities`, `%s start --request route.json`, `%s list`, `%s status JOB_ID`, `%s events --after N JOB_ID`, `%s send JOB_ID` via stdin, `%s resume JOB_ID` via stdin, `%s cancel JOB_ID`, and `%s evidence JOB_ID`.\n", runplane.Binary, runplane.Binary, runplane.Binary, runplane.Binary, runplane.Binary, runplane.Binary, runplane.Binary, runplane.Binary, runplane.Binary, runplane.Binary)
...
func validateRenderedPath(relative string) (string, error) {
	...
	if len(parts) < 2 || (parts[0] != "codex" && parts[0] != "claude" && parts[0] != "antigravity") {
