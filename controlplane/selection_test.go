...
func validSelectionEnvelope(t *testing.T) SelectionEnvelope {
	...
	envelope := SelectionEnvelope{
		...
		ProposedLenses: []LensProposal{{RoleID: "technical-go", Kind: RoleKindTechnical, Reason: "Go files are owned"}},
		Refinements:    []SelectionRefinement{}, SelectedLeafIDs: []string{"technical-go"},
		Invocations: []InvocationEdge{
			...
			{ParentRoleID: "workflow-executor", ParentKind: RoleKindWorkflow, ChildRoleID: "technical-go", ChildKind: RoleKindTechnical},
		},
...
func TestSelectionRejectsDirectRootToLeaf(t *testing.T) {
	...
	envelope.Invocations[1] = InvocationEdge{
		...
		ChildRoleID: "technical-go", ChildKind: RoleKindTechnical,
	}
	if err := SealSelection(&envelope); err != nil {
		t.Fatal(err)
	}
...
func TestSelectionRejectsWrongRefinementOwner(t *testing.T) {
	...
	envelope.Refinements = []SelectionRefinement{{
		ActorRoleID: "workflow-planner", RoleID: "technical-go", Kind: RoleKindTechnical,
