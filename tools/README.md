...
`workcell-flow` is an independent Workcell tool for recording and viewing a run. It has its own versioned JSON format and no dependency on Herdr or its Flow plugin. Python 3.11+ on Linux or macOS is sufficient; there are no third-party runtime packages.
...
```sh
workcell-flow init 'Add CSV export'
workcell-flow milestone discovery 'Define export behavior'
workcell-flow milestone delivery 'Implement and verify'
workcell-flow task add requirements 'Confirm export fields' --milestone discovery --phase spec
workcell-flow task add implementation 'Implement CSV export' --milestone delivery --phase build --after requirements
workcell-flow view
workcell-flow view --watch
workcell-flow serve
```

Before installation, use `python3 /absolute/path/to/tools/workcell-flow`. The complete executable also ships inside each plugin under `tools/workcell-flow`.

The default store is `.workcell/` relative to the current directory. Invoke from the workspace root, or pass `--dir /absolute/path/to/store` before the subcommand. Workcell's repository already ignores `.workcell/`; add that exclusion during target repo setup when appropriate.
...
```sh
workcell-flow phase spec
workcell-flow agent main --role main --harness codex --phase spec --activity 'Clarifying error cases'
workcell-flow agent scout-1 --parent main --role scout --harness claude --phase spec --activity 'Finding the existing export contract'
workcell-flow task set requirements working --agent scout-1
```
...
```sh
workcell-flow task set requirements blocked --note 'Waiting for the required CSV columns'
workcell-flow task set requirements working
workcell-flow task set requirements done --evidence 'Confirmed fields in docs/specs/spec01-20260907-csv-export.md'
workcell-flow phase build
workcell-flow task set implementation working --agent main
workcell-flow task set implementation failed --note 'Unicode export test failed'
workcell-flow task retry implementation --note 'Fix quoting for non-ASCII values' --agent main
workcell-flow task set implementation done --evidence 'python3 -m unittest: 12 passed; docs/reports/build01-20260907-csv-export.md' --verified --source-revision abc123
```
...
```bash
workcell-flow usage implement --record response-123 --agent builder \
...
```sh
workcell-flow agent main --state done --activity 'Implementation and combined checks complete'
workcell-flow finish --note 'CSV export implemented and verified'
workcell-flow archive
workcell-flow list
workcell-flow view --archive RUN_ID
workcell-flow check --archive RUN_ID
```
...
Run `workcell-flow` (or `workcell-flow view`) from the project root to open the live terminal UI. It polls `.workcell/run.json` every two seconds. When no active run exists, the terminal initially opens the latest archive and labels it ARCHIVE. Use [ / ] to switch back to the active slot or browse other archives. Use `view --archive RUN_ID` to open a completed run directly. In a source checkout, use `./tools/workcell-flow` until installed.
...
The bundled Codex plugin hooks register agent sessions and collect available model, effort, and cumulative usage metadata into the same `.workcell/run.json`. See [hook setup and limits](../hooks/README.md#codex-run-lifecycle-and-usage). This is independent of terminal/browser rendering. No personal plugin installation is implied by building bundles.

`workcell-flow session bind SESSION_ID --task TASK_ID` binds a registered native session to the currently started attempt. Use `--task -` to detach. Bind before the relevant model turn; token intervals crossing an uncertain boundary stay session-only in the agent panel. The initial session history is never retroactively charged to a task. Models and effort are observed facts, never inferred from helper profile defaults.
...
Elapsed duration freezes at an attempt's end. Last task progress measures semantic task events; report age measures stored/observed updates. Neither silence nor a large age proves a process died. Diagnostics shows the selected store, active/archive status, observed sessions, model/effort, binding, transcript cursor, and warnings. It explicitly does not infer installation status. Use `workcell-flow doctor` (optionally `--archive ID` or `--json`) even when no active run exists.
...
```sh
workcell-flow task add review 'Review the implementation' --kind review --after build
# After the review is settled, record its finding as the retry cause:
workcell-flow task retry build --cause review.a1 --note 'Address the review finding'
workcell-flow task progress build --done 2 --total 5 --note 'Checking edge cases'
workcell-flow task add verify 'Verify combined changes' --kind gate --after build --after review
```
...
By default, Flow resolves JJ workspace repository pointers and Git's common directory to one project `.workcell` store. Separate clones remain separate even when their remote URL matches. Existing `.workcell/run.json` is preserved; additional session runs live under `.workcell/runs/RUN_ID/run.json`. Each run archives beside its store under `archive/RUN_ID/run.json`. `--dir` explicitly overrides discovery.

The default viewer combines active runs. The browser run selector and terminal `[` / `]` select individual active or archived runs. Combined rows namespace IDs for display; use original task IDs with `--run RUN_ID` for mutations. When multiple runs exist, select the run explicitly or use the launcher's `WORKCELL_RUN_ID` environment. The legacy root run remains the default for unscoped commands when present.

Launch independent sessions from any workspace:

```sh
workcell-flow track --harness codex -- codex
workcell-flow track --harness agy -- agy
workcell-flow track --harness claude -- claude
```

Use your actual installed harness executable and arguments after `--`. To collaborate on an existing run:

```sh
workcell-flow --run RUN_ID track --harness agy -- agy
```

The launcher supplies `WORKCELL_RUN_ID` and `WORKCELL_SESSION_ID` to its child. It registers the workspace and observes process exit. Native Codex hooks join that run and collect the available telemetry; Agy/Claude receive lifecycle tracking through the launcher, but token reporting still requires their own adapter or explicit `usage` records. No model/effort is guessed from the command name.
...
The command is now `workcell-flow`. Existing `.workcell` stores and the version-1 schema marker remain readable; no archive migration is required.
