...
class CodeReviewSkillTests(unittest.TestCase):
    def test_markdown_is_default_and_preserves_verified_and_refuted_evidence(self):
        payload = (EXAMPLES / "expected-review.json").read_text()
        with tempfile.TemporaryDirectory() as tmp:
            source = Path(tmp) / "review.json"
            source.write_text(payload)
            result = run_render(source, "--source", "abc123", "--scope", "Orders API", "--limitations", "Mobile client not exercised", "--generated-at", "2026-09-06T12:00:00Z")
            self.assertEqual(result.returncode, 0, result.stderr)
            output = source.with_suffix(".md").read_text()
            self.assertEqual(source.read_text(), payload)
            self.assertFalse(source.with_suffix(".html").exists())
        self.assertTrue(output.startswith("# Review\n"))
        self.assertIn("Source: abc123", output)
        self.assertIn("Orders API", output)
        self.assertIn("Mobile client not exercised", output)
        self.assertIn("## Refuted candidates", output)
        for finding in json.loads(payload)["findings"] + json.loads(payload)["dropped"]:
            self.assertIn(finding["claim"], output)
        self.assertIn("prefixes the tenant id", output)

    def test_markdown_keeps_untrusted_content_literal(self):
        payload = json.loads((EXAMPLES / "expected-review.json").read_text())
        payload["findings"][0]["claim"] = "<script>alert(1)</script> [click](https://bad.example)"
        with tempfile.TemporaryDirectory() as tmp:
            source, target = Path(tmp) / "review.json", Path(tmp) / "report.md"
            source.write_text(json.dumps(payload))
            result = run_render(source, target, "--generated-at", "2026-09-06T12:00:00Z")
            self.assertEqual(result.returncode, 0, result.stderr)
            output = target.read_text()
        self.assertNotIn("<script>", output)
        self.assertNotIn("[click](https://bad.example)", output)
        self.assertIn("&lt;script&gt;", output)

    def test_explicit_html_and_source_overwrite_guard(self):
        with tempfile.TemporaryDirectory() as tmp:
            source = Path(tmp) / "review.json"
            original = (EXAMPLES / "expected-review.json").read_text()
            source.write_text(original)
            rendered = run_render(source, "--format", "html", "--generated-at", "2026-09-06T12:00:00Z")
            self.assertEqual(rendered.returncode, 0, rendered.stderr)
            self.assertIn("<!doctype html>", source.with_suffix(".html").read_text().lower())
            rejected = run_render(source, source)
            self.assertNotEqual(rejected.returncode, 0)
            self.assertEqual(source.read_text(), original)

    def test_shared_report_styles_stay_identical_after_relocation(self):
        skills = ROOT.parent
        review_css = (ROOT / "templates/report.css").read_bytes()
        self.assertEqual(review_css, (skills / "plan/templates/report.css").read_bytes())
        self.assertEqual(review_css, (skills / "plan/research/templates/report.css").read_bytes())
