...
All local wave builds use one final PR. Entry skills pass the integration bookmark as `base` in [`agents/handoff.md`](../../../agents/handoff.md).

See [the executable run protocol](../../../docs/build-runs.md) for complete input examples and recovery. Initialize once, with a durable directory outside the repository (shown as `$WORKCELL_RUN`):
