...
Offer serial or parallel execution when the plan contains genuinely independent tasks and the user has not already expressed a preference. An explicit parallel-build request authorizes native subagent use within active harness policy; no foreign harness is implied. Default to direct execution for small or coupled work. If delegation is unavailable, explain the limitation and continue serially when that still satisfies the request; never claim a parallel run occurred.

Before spawning, identify each task's acceptance criteria, shared interfaces, dependencies, file ownership, and verification commands. Stabilize interfaces and finish prerequisite tasks first. Choose concurrency from useful independent work and available resources, not a fixed worker count. Do not parallelize competing benchmarks or writers of the same files.
