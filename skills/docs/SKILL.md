...
4. Run applicable docs lint/build/link checks and relevant example or generated-diagram checks. Use the Workcell docs checker only when it exists in the target project and applies; do not assume this repository's tooling exists everywhere. Inspect rendered pages at useful viewport sizes. Separate source accuracy, structural validity, and visual checks.
