...
The canonical documentation standard and README contract live in the [`documenter` agent body](../../agents/bodies/documenter.md); link to that contract rather than duplicating it here. See [examples/visual-readme.md](examples/visual-readme.md) for the reference shape.

## Pass

1. **Dispatch the inventory.** Send one `documenter` agent a brief carrying the documentation goal, the changed-file list or release handoff, the allowed ownership paths, the repository root, and the canonical [`documenter` agent contract](../../agents/bodies/documenter.md). The agent inventories the documentation and reports stale, duplicate, missing, and misplaced material before writing.
...
3. **Gate the result.** The `documenter` agent runs `python3 -B skills/docs/scripts/docs_check.py <repo-root>` — and, where the repository generates its README visuals, its `render-diagrams.py --check` ([ADR 0010](../../docs/adr/0010-readme-diagrams-are-generated-svg.md)) — and returns the exact command, exit code, summary, and changed files. The gate is GREEN only when `docs_check` exits zero and the handoff shows every requested doc area covered; otherwise re-dispatch the failed area. The orchestrator reads this evidence and never judges the document contents itself.
