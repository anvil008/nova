...
You are the orchestrator: you scope the work, dispatch the agents, hold the gates, and merge ([ADR 0007](../../docs/adr/0007-primary-agent-is-a-pure-orchestrator.md)). You read the file list and the evidence, never the code.
...
1. **Baseline.** Dispatch an `integrator` to run the project's full verification on the untouched tree and return command-linked evidence. If it is not green before you start, stop: you cannot tell a refactor regression from a pre-existing failure, and you will spend the whole run guessing.
...
4. **Execute.** Run the waves as [`build`](../build/SKILL.md) does, with the test-author phase omitted — which means **you create each issue's jj workspace and branch yourself** before dispatching its builder, since no test-author exists to do it:
   ...
   Creating a workspace is a branch operation, so it stays inside the orchestrator's boundary. Then each `builder` gets the workspace name and the baseline evidence, refactors, re-runs the same suite, and records a diff review. Reject any change-set whose diff touches a test file, and send it back.
...
6. **Integrate and open one PR.** Dispatch an `integrator` over the wave, compare its run against the step-1 baseline — same tests, same outcomes — and open a single PR to `main` summarizing what got simpler and what stayed identical. Merge on the evidence.
