...
- **Emit structured handoff**: Return one `anvil.agent-handoff/v1` record ([contract](../handoff.md)) with `branch`, `changeId`, `commitId`, `pr: null`, `workspace`, `changedFiles`, tests with command IDs, runtime proof evidence, review passes, result, and disposition.
...
8. Return one `anvil.agent-handoff/v1` record ([contract](../handoff.md)) with `branch`, `changeId`, `commitId`, `pr: null`, `workspace`, `changedFiles`, tests (every entry cites its `commandId`), the runtime evidence, the review passes and their outcome, result, and disposition.
