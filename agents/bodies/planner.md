...
4. **Author each issue's `acceptanceTests` — its TDD Definition of Done.** These are specifications, not runnable code: `name`, `kind` (`unit`/`integration`/`e2e`), and `oracle`, with `testPath` and `stub` optional. Write each `oracle` as an observable pass condition that a `test-author` can turn into a real failing assertion without guessing what you meant. "Handles bad input gracefully" is not an oracle; "rejects a negative quantity with a 422 and leaves the cart unchanged" is. Never write test files yourself.
