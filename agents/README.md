...
Skills remain the main workflow entrypoints. These helpers are reusable native configurations for bounded work inside those workflows. None is required for a direct, single-conversation task. The former research role is replaced by scout; no scout/research skill is added. The scout also handles [bulk-read routing](../instructions/read-routing.md) without a separate reader role.
...
Native source files are `agy/NAME/agent.md`, `claude/NAME.md`, and `codex/NAME.toml`. Personal destinations are respectively `~/.gemini/config/agents/NAME/agent.md`, `~/.claude/agents/NAME.md`, and `~/.codex/agents/NAME.toml`. Agy and Claude definitions ship in native plugin agents/. Codex definitions ship as explicit setup resources; see ../docs/install.md. Reviewer settings inherit. Scout and implementer use the model selections below; no permission bypass is prescribed.
