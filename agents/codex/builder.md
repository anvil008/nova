...
5. **Review the change before any PR exists — at most two passes.** Once the suite is GREEN and the change is proven to run, hand the change-set to a read-only `reviewer` with Codex's `spawn_agent` tool — the `reviewer` agent the workcell plugin ships — one lens per spawn, and act on what comes back:
