...
5. **Parallel documentation.** For each wave, dispatch a `documenter` via `spawn_subagent` on branch `doc/<slug>` with docs-only ownership to update guides and reference docs in parallel. Documentation merges inside a combined GREEN: the `documenter` reports `python3 skills/docs/scripts/docs_check.py <repo-root>` — plus `render-diagrams.py --check` ([ADR 0010](../../runtime/docs/adr/0010-readme-diagrams-are-generated-svg.md)) where README visuals are generated — and the `integrator` reports the combined suite.
