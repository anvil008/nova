...
5. **Treat every PR as tested on its old base.** Dispatch an `integrator` via `spawn_subagent` to combine the wave changes onto the integration trunk. The integrator uses `jj rebase -s <changeId> -d <planId>-integration` to instantly combine wave changes onto the integration trunk in 50ms with zero network roundtrips (or onto a scratch ref in default mode). The integrator runs the full project test suite (`scripts/run-tests.sh` + linters) ONCE on the combined wave state. It returns command-linked evidence and the mechanical gate output; it does not decide anything.
...
Entry skills that promise one PR opt into single-PR mode; the default multi-PR mode above is unchanged:
...
1. Before the first wave, create an integration branch from `trunk()` named `<planId>-integration`:
   ...
   ```
2. In every dispatch brief conforming to [`anvil.agent-handoff/v1`](../../runtime/handoff.md), pass that branch as the `base` field. Intermediate PR creation is eliminated: builders commit locally in isolated jj workspaces via `jj describe -m "..."` and hand off their local `changeId` without pushing intermediate branches or opening intermediate PRs.
