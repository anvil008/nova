...
3. **Dispatch each issue in two phases, in order.** The separation is the point: the agent that defines "done" is not the agent judged against it.
   ...
   - **Phase 2 — `builder`.** Dispatch via `spawn_subagent` only after the seal exists. It enters the same workspace, implements, verifies GREEN with `tdd-guard verify --green-command`, self-reviews, opens the PR, and tears the workspace down. It cannot edit the sealed tests: the guard denies those edits outright.
...
1. Before the first wave, create an integration branch from `trunk()` named `<planId>-integration`:
   ...
   ```
2. In every dispatch brief conforming to [`anvil.agent-handoff/v1`](../../runtime/handoff.md), pass that branch as the `base` field. Builders open their per-issue PRs against `<planId>-integration`, never `main`.
