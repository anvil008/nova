#!/usr/bin/env python3
"""Render a verified review as Markdown, or explicitly requested self-contained HTML."""
...
def lens_chips(findings: list[dict], declared: list[str]) -> tuple[str, int]:
    used = sorted({finding["lens"] for finding in findings} | set(declared), key=lambda lens: LENSES.index(lens))
    if not used:
...
def markdown_text(value: object) -> str:
    """Keep source-provided prose literal rather than interpreting it as markup."""
    text = re.sub(r"([\\`*_\[\]#|])", r"\\\1", html.escape(str(value), quote=False))
    return re.sub(r"(?m)^(\s*)([-+]|\d+[.)])(?=\s)", r"\1\\\2", text)


def render_markdown(review: dict, context: dict) -> str:
    lines = [
        f"# {markdown_text(context['title'])}", "",
        f"**Verdict:** {review['verdict']}", "",
        f"Repository: {markdown_text(context['repo'])}  ",
        f"Subject: {markdown_text(context['subject'])}  ",
        f"Source: {markdown_text(context['source'])}  ",
        f"Comparison base: {markdown_text(context['base'])}  ",
        f"Generated: {markdown_text(context['generatedAt'])}", "",
        f"{review['verifiedCount']} verified finding(s) from {review['inputCount']} observations; "
        f"{review['candidateCount']} candidates after deduplication; {review['droppedCount']} refuted.", "",
        "## Coverage", "",
        f"Scope: {markdown_text(context['scope'])}", "",
        "Lenses: " + (", ".join(context["lenses"]) or "none recorded"), "",
        f"Limits: {markdown_text(context['limitations'])}", "",
    ]
    for field, title, prefix in (("findings", "Findings", "F"), ("dropped", "Refuted candidates", "D")):
        lines.extend([f"## {title}", ""])
        if not review[field]:
            lines.extend(["No verified findings in the recorded scope." if field == "findings" else "No refuted candidates.", ""])
        for index, finding in enumerate(review[field]):
            lines.extend([
                f"### {identifier(index, prefix)} · {finding['severity']} · {markdown_text(finding['claim'])}", "",
                f"Location: {markdown_text(finding['file'])}:{finding['line']}  ",
                f"Lens: {finding['lens']} · Confidence: {finding['confidence']:.0%}", "",
                f"Failure scenario: {markdown_text(finding['failureScenario'])}", "",
                f"Refutation attempt: {markdown_text(finding['verification']['refutationAttempt'])}", "",
                f"Evidence: {markdown_text(finding['verification']['evidence'])}", "",
            ])
    lines.extend([
        "## Method", "",
        "Findings were deduplicated deterministically and independently challenged. "
        "Refuted claims are retained above. This verdict applies only to the recorded source and scope; "
        "it does not authorize implementation or tracker changes.", "",
    ])
    return "\n".join(lines)
...
def build_context(args: argparse.Namespace) -> dict:
    ...
    return {
        "title": args.title.strip() or "Review",
        "repo": repo,
        "subject": args.subject.strip(),
        "base": args.base.strip(),
        "source": args.source.strip(),
        "scope": args.scope.strip() or args.subject.strip(),
        "limitations": args.limitations.strip() or "No additional coverage information recorded.",
        "lenses": lenses,
...
def main() -> int:
    ...
    parser.add_argument("output", type=Path, nargs="?", help="defaults to <review>.md; .html explicitly selects HTML")
    parser.add_argument("--format", choices=("markdown", "html"), help="default: Markdown unless output explicitly ends in .html")
    ...
    parser.add_argument("--source", default="—", help="reviewed immutable source commit")
    parser.add_argument("--scope", default="", help="paths, interfaces, or flows actually reviewed")
    parser.add_argument("--limitations", default="", help="unrun checks, coverage gaps, and remaining uncertainty")
    ...
    try:
        ...
        format_name = args.format or ("html" if args.output and args.output.suffix.lower() in {".html", ".htm"} else "markdown")
        output = args.output or args.review.with_suffix(".html" if format_name == "html" else ".md")
        if output.resolve() == args.review.resolve():
            raise ReviewError("output must not overwrite the source review JSON")
        rendered = render(review, context) if format_name == "html" else render_markdown(review, context)
