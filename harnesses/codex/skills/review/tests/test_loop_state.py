...
class ReviewProgressTests(unittest.TestCase):
    ...
    def test_init_has_no_default_iteration_limit(self):
        result = run(self.state, "init")
        self.assertEqual(result.returncode, 0, result.stderr)
        output = json.loads(result.stdout)
        self.assertEqual(output["branch"], "loop-branch")
        self.assertIsNone(output["maxIterations"])
    ...
    def test_repeated_findings_are_a_signal_not_an_automatic_stop(self):
        """The orchestrator decides whether another approach can resolve the findings."""
        run(self.state, "init")
        ...
        self.assertTrue(output["continue"])
        self.assertEqual(output["progress"], "unchanged")
        self.assertEqual(output["latest"]["repeatedCount"], 2)
    ...
    def test_explicit_user_limit_is_preserved_even_when_findings_change(self):
        """An explicit user limit remains binding despite apparent progress."""
    ...
    def test_accepts_a_real_verified_review(self):
        run(self.state, "init")
    ...
    def test_no_default_limit_survives_many_unchanged_reports(self):
        run(self.state, "init", "--branch", "feature/export")
        source = self.write_review("same.json", review([finding("still unresolved")]))
        for _ in range(12):
            result = run(self.state, "record", source)
            self.assertEqual(result.returncode, 0, result.stderr)
        output = json.loads(result.stdout)
        self.assertEqual(output["status"], "running")
        self.assertEqual(output["latest"]["repeatedCount"], 12)
        self.assertIsNone(output["maxIterations"])

    def test_orchestrator_records_stall_and_resumes_without_losing_history(self):
        run(self.state, "init")
        source = self.write_review("r.json", review([finding("compatibility unclear")]))
        run(self.state, "record", source)
        result = run(self.state, "decide", "--status", "stalled", "--reason", "Needs compatibility decision")
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(json.loads(result.stdout)["decision"]["reason"], "Needs compatibility decision")
        self.assertNotEqual(run(self.state, "record", source).returncode, 0)
        resumed = run(self.state, "decide", "--status", "running", "--reason", "Preserve old clients")
        self.assertEqual(resumed.returncode, 0, resumed.stderr)
        output = json.loads(run(self.state, "record", source).stdout)
        self.assertEqual(output["iteration"], 2)
        self.assertEqual(len(json.loads(self.state.read_text())["decisions"]), 2)

    def test_decision_cannot_bypass_explicit_user_limit(self):
        run(self.state, "init", "--max-iterations", "1")
        source = self.write_review("r.json", review([finding("unresolved")]))
        run(self.state, "record", source)
        result = run(self.state, "decide", "--status", "running", "--reason", "Try again")
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("explicit iteration limit", result.stderr)
