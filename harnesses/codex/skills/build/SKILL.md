...
3. **Dispatch each issue in two phases, in order.** The separation is the point: the agent that defines "done" is not the agent judged against it.
   ...
   - **Phase 2 — `builder`.** Dispatch via `spawn_agent` only after the seal exists. It enters the same workspace, implements, verifies GREEN, self-reviews, opens the PR, and tears the workspace down. It cannot edit the sealed tests: the guard denies those edits outright.
