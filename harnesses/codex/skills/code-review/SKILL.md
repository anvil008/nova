...
The fan-out count equals the applicable lenses, never a fixed N. Spawn one read-only `reviewer` per selected lens in parallel via `spawn_agent`. Give each reviewer the same change-set and exactly one lens. Reviewers return structured findings and never edit.
...
2. Run an independent adversarial verification of every surviving candidate. Use a fresh read-only reviewer via `spawn_agent` that did not originate the candidate, assign its lens and exact claim, and require a skeptic pass that tries to refute it against the code and concrete failure scenario.
