...
Today that is Claude (`/loop <interval> <prompt>`). Codex and Antigravity have no equivalent,
...
1. Put the work on its own branch. Never run this loop on `main`. The repository is
   jj-colocated (see the [`jj`](../jj/SKILL.md) skill), so the branch is a bookmark on the current change:
...
1. **Review.** Run the [`code-review`](../code-review/SKILL.md) skill over the diff between
   ...
   `reviewer` per lens via `spawn_agent`, then run the independent adversarial verification. Merge to a single
...
3. **Fix — only if `continue` is true.** Dispatch the `builder` via `spawn_agent` to fix the reported findings,
