...
2. **Dispatch consolidation.** Dispatch a single `documenter` via `spawn_agent` with a brief conforming to [`anvil.agent-handoff/v1`](../../runtime/handoff.md) whose `ownership` is restricted to the resolved namespace path.
