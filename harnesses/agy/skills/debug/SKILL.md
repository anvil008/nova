...
Dispatch specialists with Antigravity's `invoke_subagent` and run VCS and gate commands with `run_command`. Use the configured roles in `agents/models.json` and [`anvil.agent-handoff/v1`](../../runtime/handoff.md). Native runtime capacity affects scheduling; it does not prescribe a workflow team size.
