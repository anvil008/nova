...
Do not ask what the repository can tell you. Before the interview, dispatch a `researcher` agent via `invoke_subagent` for a repository picture — manifests, architecture seams, and existing conventions — then state what its evidence suggests and ask the human to correct it. Batch the questions in one pass, mark which are blocking, and offer your recommendation for each.

## Procedure

1. **Interview** as above. Write the answers down; they are the planner's brief.
2. **Plan.** Dispatch the `planner` via `invoke_subagent` with the goal and the answers. It investigates read-only and returns the folio, sidecar, and per-issue `acceptanceTests`. If it returns `needs-decision`, that is a question the interview missed: put it to the human and re-dispatch rather than answering on their behalf.
3. **Approve.** Present the folio and stop for explicit human approval, then write the milestone and issues yourself via `run_command`.
...
Never start planning on an unanswered blocking question, and never encode an unresolved decision into the plan. If the answers reveal the request is really a bug fix or cleanup, route to [`code-analysis`](../code-analysis/SKILL.md) or [`code-refactor`](../code-refactor/SKILL.md).

Based on the requirements and constraints above, execute the new-feature workflow systematically.
