...
Run the target harness's command below as a terminal command. It starts a separate external process: it is not an Antigravity subagent and nothing in this session manages its lifetime, so wait for it and collect its output yourself.
