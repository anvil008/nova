...
1. **Ask whether the project opted in.** This is a command, never a directory test: the store lives outside every repository, so nothing in the working copy can answer it. Run via `run_command`:

   ```bash
   python3 -B skills/wiki/scripts/wiki.py status --repo .
   ```

   It prints the resolved `projectKey`, the namespace path, and `present`. When `present: false` comes back the project has no namespace, and the loop ends immediately — no record, no dispatch, no extra tokens, and nothing changed about what the human was handed. An absent namespace (or where `present` is `false`) ends the loop without recording any trace and without dispatching any agent. The same command refuses a repository in eval mode, which ends this step the same way.
...
Based on the requirements and constraints above, execute the review-fix-loop workflow systematically.
