...
Dispatch planner agents with Antigravity's `invoke_subagent` tool and use the native lifecycle and messaging tools available in the session. Planner-owned research uses nested dispatch only when supported; otherwise the orchestrator dispatches assignments and returns findings to the planner. Run commands with `run_command`. Teamwork is optional and never required. Use `agents/models.json` and [`anvil.agent-handoff/v1`](../../runtime/handoff.md).
