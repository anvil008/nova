...
This orchestrator splits the goal into areas, dispatches one read-only `researcher` agent per area in parallel, and owns synthesis. Per the Gemini 3.7 Flash guide, place critical constraints first, demand evidence-based reporting without premature filtering, and retain all conflict positions.
