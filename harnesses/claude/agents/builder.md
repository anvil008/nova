...
- **Emit structured handoff**: Return one `anvil.agent-handoff/v1` record ([contract](../runtime/handoff.md)) with `branch`, `changeId`, `pr: null`, `workspace`, `changedFiles`, tests with command IDs, runtime proof evidence, review passes, result, and disposition.
