...
1. **reproduce symptom**: Establish a deterministic reproduction command triggering the failure on demand.
2. **isolate cause**: Trace and identify the root cause with concrete `file:line` evidence.
3. **RED seal**: A `specifier` agent encapsulates the failure in a clean test and seals it via `tdd-guard seal`.
4. **GREEN**: A `builder` agent implements the fix in an isolated workspace, confirming tests pass without modifying the seal.
5. **review**: Multi-lens review verifies the fix addresses the root cause without introducing regressions.
6. **pull request**: A single pull request to `main` is opened documenting symptom, cause, and verification.
