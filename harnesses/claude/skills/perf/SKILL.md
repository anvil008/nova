...
1. **benchmark baseline**: Measure existing performance using the project's benchmark harness across multiple iterations to establish median and spread.
2. **optimization**: Implement targeted, behaviour-preserving performance improvements under a `kind: baseline` seal.
3. **benchmark comparison**: Re-measure using identical harness, machine, and iterations to confirm statistically significant gains outside noise.
4. **review**: Peer review verifies correctness preservation and confirms no readability regressions for insignificant gains.
...
2. **Correctness baseline.** Dispatch an `integrator` with a brief conforming to [`anvil.agent-handoff/v1`](../../runtime/handoff.md) and carrying `mode: baseline`: run the documented verification on the untouched tree at `base`, return command-linked evidence, and perform no merge. Stop unless that baseline is green.
