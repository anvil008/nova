...
Run the target harness's native headless command in the requested workspace. For Claude Code,
the compatible launch shape is:

```bash
claude -p "<task prompt>" --model <model> --effort <effort> --dangerously-skip-permissions
```
