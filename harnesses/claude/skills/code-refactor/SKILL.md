...
1. **green baseline**: Establish that all existing verification tests pass completely before modifying code.
2. **behaviour-preserving change**: Author changes strictly simplifying architecture, consolidating duplicates, or removing dead paths without touching tests or public behaviour.
3. **GREEN**: Confirm that all existing tests remain green post-change with `tdd-guard verify --green-command`.
4. **review**: Independent review verifies strict behaviour preservation and that no test files or public contracts changed.
5. **pull request**: A single pull request to `main` is opened documenting simplifications and presenting clean verification evidence.
