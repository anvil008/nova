...
Dispatch specialists with Claude Code's `Agent` tool and use `Bash` for VCS and validation commands. Use [anvil.agent-handoff/v1](../../runtime/handoff.md) and configured specialist roles. Resolve bundled helper paths from this skill installation; the `skills/...` command examples are relative to the Workcell package root, while the target repository and run directory are supplied by the brief.
...
Each reviewer returns the [reviewer contract](../../agents/reviewer.md). Save one `{lens, findings}` envelope per lens, even when a specialist covers several lenses. Every finding needs a reachable failure scenario, a source location, severity, and confidence. Distinguish demonstrated defects from design preferences and unknowns.
