...
Implemented the five gaps prioritized in the Herdr comparison, using Nova's own state and renderer:
...
- Diagnostics explain the selected store/run, observed sessions, model/effort, task binding, transcript cursor, and warnings. `nova-dagr doctor` works with missing run data and does not claim to know personal hook installation status.
