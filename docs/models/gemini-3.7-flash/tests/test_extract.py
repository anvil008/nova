"""Acceptance tests for the Gemini 3.7 Flash prompting guide (#150)."""
...
class GeminiGuideAcceptanceTests(unittest.TestCase):
    def test_gemini_guide_provenance_and_strategy(self):
        ...
        self.assertEqual(
            ...
            set(OFFICIAL_URLS),
            "The extract must identify exactly the two official Google sources.",
        )
        ...
        for heading_terms, expected_source, claims in strategies:
            ...
            with self.subTest(strategy=label):
                ...
                self.assertIn(
                    expected_source,
                    section,
                    f"The {label} section must attribute its official Google source.",
                )
                self.assertRegex(
                    section,
                    r"(?m)^>\s+\S",
    ...
    def test_gemini_guide_is_live_fresh(self):
        ...
        self.assertEqual(
            live.returncode,
            0,
            "The checker must refetch both official Google sources and report the "
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                clean.returncode,
                0,
                "Fixtures normalized from the two live sources must be fresh.\n"
