...
The global installation command (`scripts/install-harness.sh --install`) links these plugin wrappers into their respective harness plugin directories (e.g., `~/.claude/plugins/nova`).
...
- **Cleaner Global Namespace:** Agents and skills are grouped under the `nova` plugin, preventing clutter in the harness root directories.
- **Project-Local Tooling:** It becomes trivial to install Nova per project, ensuring the project workspace has the exact agents and hooks needed without relying on a global install.
