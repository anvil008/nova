...
Execution variants are data in `agents/handoff.md`, not hidden implications in orchestration prose. Dispatch briefs carry `mode: baseline`, `mode: refactor`, or `mode: loop` with their fixed semantics, and carry runtime review setup through `devServer`. Behaviour-preserving performance and refactor work uses a `kind: baseline` test seal instead of fabricating RED: the orchestrator creates the workspace and branch, an integrator proves `baselineCommand` is green and seals `sealedTests`, and only then does the refactor builder receive the workspace. Observable behaviour changes return to the ordinary planner and test-author path.

The baseline seal is not a parallel gate. `tdd-guard seal --green-baseline` requires exit zero rather than the RED seal's non-zero result, then uses the same sealed-path edit denial, command binding, post-seal GREEN verification, diff review, handoff, status, and Stop machinery. A baseline seal cannot be resealed because a behaviour-preserving refactor never amends its tests.

## Consequences

The primary agent can operate every workflow without inspecting target-project contents, while dispatched agents receive all information needed to select their contract. Entry workflows that promise one PR now share one integration mechanism, and GitHub issue closure remains correct on the default branch. Refactor builders can satisfy the ordinary mechanical Stop gate without being allowed to weaken the green oracle they inherited.
