...
[`scripts/bootstrap-eval.sh`](../scripts/bootstrap-eval.sh) and read by `build-guard`. It is
scoped to the task repository: the bootstrap refuses to mark the Workcell source tree, and the
guard resolves the marker from the repository each command targets, so a workcell checkout on
the same machine is never itself in eval mode.
